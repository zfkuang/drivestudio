...
def main(args):
    ...
    for step in metric_logger.log_every(all_iters, cfg.logging.print_freq):
        ...
        if step % cfg.logging.vis_freq == 0:# and cfg.logging.vis_freq > 0:
            ...
            # output_rgbs = []
            # for st_idx in range(0, dataset.num_img_timesteps, 10):
            #     vis_indices = [
            #         idx * dataset.pixel_source.num_cams + i
            #         for i in range(dataset.pixel_source.num_cams)
            #         for idx in range(st_idx, st_idx + 10)
            #     ]
            #     with torch.no_grad():
            #         render_results = render_images(
            #             trainer=trainer,
            ...
            #         )
            #     output_rgbs.extend(render_results["rgbs"])
            #     del render_results
            logger.info("Visualizing all timesteps...")
            render_results = render_images(
                trainer=trainer,
                dataset=dataset.full_image_set,
            ...
            # output_rgbs = torch.stack(output_rgbs, dim=0)
            # output_rgbs = {"rgbs": output_rgbs}
            video_output_pth = os.path.join(cfg.log_dir, "videos", f"step_{step}_rgbs.mp4")
            vis_frame_dict = save_videos(
                render_results,
                video_output_pth,
                layout=dataset.layout,
                num_timestamps=dataset.num_img_timesteps,
                keys=["rgbs", "gt_rgbs", "rgb_error_maps"],
                num_cams=dataset.pixel_source.num_cams,
                save_seperate_video=cfg.logging.save_seperate_video,
                fps=cfg.render.fps,
                verbose=True,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--entity", default="zfkuang", type=str, help="wandb entity name")
